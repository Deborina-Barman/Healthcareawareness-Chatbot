...
# ✅ Use Streamlit secrets for Pinecone credentials
...
@st.cache_resource
def load_model():
    st.info("🔄 Loading FLAN-T5 model and pipeline...")
    tokenizer = AutoTokenizer.from_pretrained(MODEL_ID)
    model = AutoModelForSeq2SeqLM.from_pretrained(MODEL_ID)
    pipe = pipeline("text2text-generation", model=model, tokenizer=tokenizer, max_new_tokens=512, temperature=0.3)
